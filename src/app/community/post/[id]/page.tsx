          ...
          {loadingComments ? (
            ...
            <div className="space-y-3 mb-4">
              {comments.map(comment => (
                <DetailCommentThread key={comment.id} comment={comment} depth={0} />
              ))}
            </div>
          )}

          {/* Add comment or guest CTA */}
          {hasCompleteProfile ? (
            ...
            <div className="pt-3 border-t border-gray-200 text-center">
              <p className="text-sm text-gray-500 mb-2">Join free to like and comment</p>
              <Link
                href="/login"
...
/** Recursive comment thread component for post detail page */
function DetailCommentThread({ comment, depth }: { comment: Comment; depth: number }) {
  const maxDepth = 3
      ...
      {comment.replies && comment.replies.length > 0 && (
        <div className="mt-2 space-y-2">
          {comment.replies.map(reply => (
            <DetailCommentThread key={reply.id} comment={reply} depth={depth + 1} />
          ))}
        </div>
      )}
    </div>
