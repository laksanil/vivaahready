...
export async function POST(request: NextRequest) {
  try {
    ...
    // Add user reply to notifications timeline (read by default).
    try {
      const sentAt = new Date(userTimestamp)
      await prisma.notification.create({
        data: {
          userId: session.user.id,
          type: 'support_user_message',
          title: 'Your message to Admin',
          body: content.trim().slice(0, 220) || 'Your reply was sent.',
          url: '/admin-messages',
          read: true,
          readAt: sentAt,
          data: JSON.stringify({
            messageId,
            context: supportMessage.context || 'general',
