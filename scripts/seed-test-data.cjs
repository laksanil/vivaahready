...
async function main() {
  ...
  // Step 1: Update Lakshmi's profile to approved (skip if not found on this DB)
  console.log("1. Updating Lakshmi's profile to approved...");
  const lakshmiProfile = await prisma.profile.findUnique({ where: { userId: LAKSHMI_USER_ID } });
  if (lakshmiProfile) {
    ...
    console.log("   Skipped: Lakshmi's profile not found on this database\n");
  }
  ...
  // Step 3: Create Match records (only if Lakshmi exists on this DB)
  const lakshmiUser = await prisma.user.findUnique({ where: { id: LAKSHMI_USER_ID } });
  const maleUserIds = testUsers.filter((u) => u.profile.gender === "male").map((u) => u.id);

  if (!lakshmiUser) {
    console.log("3. Skipping Match records - Lakshmi's user not found on this database");
    console.log("4. Skipping DeclinedProfile records - Lakshmi's user not found on this database\n");
  } else {
